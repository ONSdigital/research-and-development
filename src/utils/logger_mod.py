...
def get_logging_level_command(lev_from_conf):
    ...
    loglevels_dict = {"DEBUG": logging.DEBUG,
                      "INFO": logging.INFO,
...
def finishing_time(func, enter_time):
    ...
    logger.debug(f"Running the {func.__name__} function took: "
...
def df_change_wrap(func):
    ...
    This wrapper calculates the differences between the input dataframe and the output dataframe,
    helping to keep a record of changes to the dataframe. This wrapper should decorate the main
    method of a class, e.g. for the `DuplicateWaves` class, this decorate is applied to the
    `duplicate_data()` method. The method this is applied to must have must have `self` as an
    argument and must operate on and then return self.vf_df. The class as a whole also requires
...
def df_measure_change(df, rows_before, cols_before, table_config):
    ...
    if table_config == "Table":
        ...
        tbl = TableLogger(columns='df Changes,Rows,Columns',
