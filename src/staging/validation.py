...
def check_pcs_real(
    ...
    """Checks if postcodes are real against a masterlist of actual postcodes.
    ...
    This formatting is applied to a copy dataframe so the original is
    unchanged.
    ...
    Args:
        df (pd.DataFrame): The DataFrame containing the postcodes.
        check_real_df (pd.DataFrame): The DataFrame excluding invalid postcodes
            , to run the comparison against
        postcode_masterlist (pd.DataFrame): The dataframe containing the
            correct postocdes to check against
