...
class TestSaveDetailedCSV(object):
    ...
    def test_save_detailed_csv_defence(self, tmp_path, write_csv_func, test_frame):
    ...
    def test_save_detailed_csv(self, tmp_path, test_frame, write_csv_func):
        ...
        assert pd.read_csv(fpath).equals(test_frame), "Saved data not as expected."
...
class TestOutputIntramByItl(object):
    ...
    def create_dummy_config(self, path: pathlib.Path, year: int):
        ...
        config = {
            ...
            "outputs_paths": {"outputs_master": path},
        }
        return config
    ...
    def test_output_intram_by_itl_gb(
        ...
        assert output_dfs[keys[0]].equals(
            gb_itl1_output
        ), "GB ITL1 Output Not as Expected."
        assert output_dfs[keys[1]].equals(
            gb_itl2_output
