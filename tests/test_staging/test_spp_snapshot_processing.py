...
def dummy_data() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    contributor_input = pd.DataFrame(data=contributor_data, columns=contributor_schema.keys()).astype(contributor_schema)
    ...
    responses_data = [
            [101, 202012, 1, "A", 2020, "A", 2020, 200, 0, ""],
            [101, 202012, 1, "A", 2020, "A", 2020, 201, 50, ""],
            [101, 202012, 1, "A", 2020, "A", 2020, 202, 100, ""],
            [102, 202012, 1, "A", 2020, "A", 2020, 200, 75, ""],
            [102, 202012, 1, "A", 2020, "A", 2020, 201, 25, ""],
            [102, 202012, 1, "A", 2020, "A", 2020, 202, 65, ""]
    ]
    
    responses_input = pd.DataFrame(data=responses_data, columns=responses_schema.keys()).astype(responses_schema)
...
def expected_output():
    ...
    expected_data =  [
        ...
        [103, 202012, 1, np.nan, np.nan, np.nan]
    ]
    
    expected_output = pd.DataFrame(data=expected_data, columns=expected_schema.keys()).astype(expected_schema)
